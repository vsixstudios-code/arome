...
import { Cloud, MapPin, Loader2, Plane, Search, Sun, CloudRain, Snowflake, Moon, Sparkles } from 'lucide-react';
...
export default function Home() {
  ...
  const [hasRequested, setHasRequested] = useState(() => {
    // Check if user already got a recommendation today
    const cacheDate = sessionStorage.getItem('arome_daily_rec_date');
    const today = new Date().toDateString();
    return cacheDate === today && !!sessionStorage.getItem('arome_daily_rec');
  });

  // Load from cache on mount if available — no API call
  useState(() => {
  ...
  async function fetchRecommendation(lat: number | null, lon: number | null) {
    try {
      ...
      if (err.message?.includes('429') || err.status === 429 || err.message?.includes('RESOURCE_EXHAUSTED')) {
        setRecommendation({
          recommendationMarkdown: '### Daily Limit Reached\n\nYou\'ve used today\'s AI recommendations. Come back tomorrow for a fresh suggestion — or use the **Ask AI** tab for more personalized advice.',
        });
      } else {
        setRecommendation({ recommendationMarkdown: 'Could not load recommendation. Please try again.' });
      }
    } finally {
      setLoading(false);
    }
  ...
  async function fetchTravelRecommendation() {
    ...
    try {
      ...
      console.error(err);
      if (err.message?.includes('429') || err.status === 429 || err.message?.includes('RESOURCE_EXHAUSTED')) {
        setTravelRec('Daily limit reached. Please try again tomorrow or use the Ask AI tab.');
      } else {
        setTravelRec('Failed to load travel recommendation. Please try again.');
      }
    } finally {
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <header className="mb-8 mt-4 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-serif font-medium tracking-tight" style={{ color: 'var(--color-text-primary)' }}>
            Arome
          </h1>
      ...
      <section className="rounded-2xl p-6" style={cardStyle}>
        ...
        {/* Not yet requested — show a clean prompt button */}
        {!hasRequested && !recommendation && !loading && (
          <div className="flex flex-col items-center py-8 text-center">
            <p className="text-sm mb-6 leading-relaxed" style={{ color: 'var(--color-text-secondary)' }}>
              {greeting}{user?.firstName ? `, ${user.firstName}` : ''}. Ready to find your scent for today?
            </p>
            <button
              onClick={handleGetRecommendation}
              className="btn-primary flex items-center gap-2 px-8 py-3.5 rounded-2xl font-semibold shadow-md"
            >
      ...
      <section className="mt-10 mb-8">
        <h2 className="text-xl font-serif font-medium mb-5" style={{ color: 'var(--color-text-primary)' }}>
          Wardrobe Insights
        </h2>
        <div className="grid grid-cols-2 gap-4">
          {[
            ...
            >
              <span className="text-4xl font-light tracking-tight" style={{ color: 'var(--color-text-primary)' }}>
                {value}
              </span>
